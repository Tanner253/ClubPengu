...
Enable **real-money wagering on any Solana token** for P2P minigames — a first-of-its-kind feature that transforms casual gaming into competitive stakes play. Players can wager USDC, BONK, memecoins, or any SPL token on Card Jitsu, Tic-Tac-Toe, Connect 4, Monopoly, and Uno.

### Why x402 Protocol

| Advantage | What It Means |
|-----------|---------------|
| **Instant Settlement** | Winner receives tokens the moment the game ends — no delays |
| **Any SPL Token** | Support USDC, BONK, custom tokens — opens doors for token partnerships |
| **Pre-Signed Authorization** | No funds locked in escrow; tokens stay in player wallets until settlement |
| **Server-Authoritative** | Cheat-proof, dispute-free — server determines winner, executes payout |
...
| **Viral Growth** | "I just won 1000 $BONK playing Card Jitsu" — shareable moments |
...
| **Compliance Ready** | Full audit trail — every wager, signature, and settlement logged |
| **Scalability** | Same architecture handles 10 or 10,000 concurrent matches |

### User Experience Highlights

1. **Challenger** picks any token, enters amount → signs once → challenge sent
2. **Opponent** sees token details + "Buy on Jupiter" link if needed → signs once → game starts
3. **Game plays** exactly as before — no UX changes during gameplay
...
└─────────────────────────────────────────────────────────────────────────────────┘
                    ...
                    ┌───────────────────────────────┐
                    │  Opens WagerModal             │
                    │  - Selects token (CA/preset)  │
                    │  - Enters wager amount        │
                    └───────────────────────────────┘
                                    │
                                    ▼
                    ┌───────────────────────────────┐
                    │  CLIENT: Validate Token CA    │
                    └───────────────────────────────┘
                           │              │
              ┌────────────┘              └────────────┐
              ▼                                        ▼
    ...
    │                     │                │                         │
    │ Error: "Invalid     │                │ Fetch metadata (symbol, │
    │ token address"      │                │ decimals) from chain    │
    │                     │                └─────────────────────────┘
    │ [FLOW ENDS]         │                           │
    └─────────────────────┘                           ▼
                                        ┌─────────────────────────┐
                                  ...
                                  ▼                                        ▼
                    ┌─────────────────────────┐              ┌─────────────────────────┐
                    │ ✗ INSUFFICIENT BALANCE  │              │ ✓ BALANCE OK            │
                    │                         │              │                         │
                    │ Error: "Need X more     │              │ Prompt wallet to sign   │
                    │ $TOKEN to wager"        │              │ payment → Player B      │
                    │                         │              └─────────────────────────┘
                    │ [Show Jupiter buy link] │                           │
                    │                         │                           ▼
                    │ [FLOW ENDS]             │              ┌─────────────────────────┐
                    └─────────────────────────┘              │ X402 createWagerPayment │
                                                            └─────────────────────────┘
                                                                   │              │
                                                      ┌────────────┘              └────────────┐
                                                      ▼                                        ▼
                                        ┌─────────────────────────┐              ┌─────────────────────────┐
                                        │ ✗ USER REJECTED SIGN    │              │ ✓ SIGNED SUCCESSFULLY   │
                                        │                         │              │                         │
                                        │ User cancelled in       │              │ Send to server:         │
                                        │ wallet popup            │              │ - wagerToken config     │
                                        │                         │              │ - signedPayload         │
                                        │ [FLOW ENDS]             │              └─────────────────────────┘
                                        └─────────────────────────┘                           │
                                                                                              ▼
┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│                                SERVER: ChallengeService.createChallenge()                                   │
└─────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
                                                            │
                    ┌───────────────────────────────────────┼───────────────────────────────────────┐
                    │                                       │                                       │
                    ▼                                       ▼                                       ▼
    ┌───────────────────────────┐        ┌───────────────────────────┐        ┌───────────────────────────┐
    │ CHECK 1: Signature Valid? │        │ CHECK 2: Balance on-chain?│        │ CHECK 3: Payload Match?   │
    │ nacl.sign.verify()        │        │ getTokenBalance()         │        │ amount, token, recipient  │
    └───────────────────────────┘        └───────────────────────────┘        └───────────────────────────┘
              │                                    │                                    │
     ┌────────┴────────┐                  ┌────────┴────────┐                  ┌────────┴────────┐
     ▼                 ▼                  ▼                 ▼                  ▼                 ▼
┌─────────┐      ┌─────────┐        ┌─────────┐      ┌─────────┐        ┌─────────┐      ┌─────────┐
│✗ INVALID│      │✓ VALID  │        │✗ DROPPED│      │✓ HAS IT │        │✗ MISMATCH│     │✓ MATCHES│
│         │      │         │        │         │      │         │        │         │      │         │
│ Error:  │      │Continue │        │ Error:  │      │Continue │        │ Error:  │      │Continue │
│"Invalid │      │         │        │"Balance │      │         │        │"Payload │      │         │
│signature│      │         │        │changed" │      │         │        │tampered"│      │         │
│"        │      │         │        │         │      │         │        │         │      │         │
│[END]    │      │         │        │[END]    │      │         │        │[END]    │      │         │
└─────────┘      └─────────┘        └─────────┘      └─────────┘        └─────────┘      └─────────┘
                                                            │
                                      ...
                                      ▼                                ▼
                            ┌─────────────────┐              ┌─────────────────┐
                            │ ✗ NONCE REUSED  │              │ ✓ FRESH NONCE   │
                            │                 │              │                 │
                            │ Error: "Replay  │              │ ✓ ALL CHECKS    │
                            │ attack detected"│              │ PASSED!         │
                            │                 │              │                 │
                            │ [LOG + END]     │              │ Challenge saved │
                            └─────────────────┘              │ Notify Player B │
                                                             │                 │
                                                             │ [CONTINUE →]    │
                                                             └─────────────────┘
...
└─────────────────────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
                    ┌───────────────────────────────┐
                    │  Inbox shows challenge:       │
                    │  - Token symbol + amount      │
                    │  - Token CA (copyable)        │
                    │  - "Buy on Jupiter" link      │
                    │  - Your balance: X $TOKEN     │
                    └───────────────────────────────┘
                                    │
            ┌───────────────────────┼───────────────────────┐
            │                       │                       │
            ▼                       ▼                       ▼
    ┌───────────────┐       ┌───────────────┐       ┌───────────────┐
    │   DECLINE     │       │    IGNORE     │       │    ACCEPT     │
    └───────────────┘       └───────────────┘       └───────────────┘
            │                       │                       │
            ▼                       ▼                       ▼
    ┌───────────────┐       ┌───────────────────┐   ┌───────────────────┐
    │ Server:       │       │ 5 min timeout     │   │ Check B's balance │
    │ status →      │       │ triggers cleanup  │   │ for wager token   │
    │ 'declined'    │       │                   │   └───────────────────┘
    │               │       │ Server:           │           │
    │ Notify A:     │       │ status → 'expired'│      ┌────┴────┐
    │ "B declined"  │       │                   │      ▼         ▼
    │               │       │ Notify A:         │ ┌─────────┐ ┌─────────────┐
    │ [FLOW ENDS]   │       │ "Challenge expired│ │✗ LOW BAL│ │✓ ENOUGH     │
    └───────────────┘       │                   │ │         │ │             │
                            │ [FLOW ENDS]       │ │"Need X  │ │Sign payment │
                            └───────────────────┘ │more"    │ │to Player A  │
                                                  │         │ └─────────────┘
                                                  │[Jupiter │        │
                                                  │link]    │        ▼
                                                  ...
                                                  │✗ REJECTED       │ │✓ SIGNED         │
                                                  │                 │ │                 │
                                                  │User cancelled   │ │Send to server   │
                                                  │                 │ └─────────────────┘
                                                  │[END]            │        │
                                                  └─────────────────┘        ▼
┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│                                SERVER: ChallengeService.acceptChallenge()                                   │
└─────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
                                                            │
                ┌───────────────────────────────────────────┼───────────────────────────────────────────┐
                │                                           │                                           │
                ▼                                           ▼                                           ▼
┌───────────────────────────────┐        ┌───────────────────────────────┐        ┌───────────────────────────────┐
│ CHECK 1: A still has balance? │        │ CHECK 2: B's signature valid? │        │ CHECK 3: Recipient correct?   │
│ Re-verify challenger funds    │        │ nacl.sign.verify()            │        │ B signed to A's wallet?       │
└───────────────────────────────┘        └───────────────────────────────┘        └───────────────────────────────┘
          │                                        │                                        │
  ┌───────┴───────┐                        ┌───────┴───────┐                        ┌───────┴───────┐
  ▼               ▼                        ▼               ▼                        ▼               ▼
┌───────┐    ┌─────────┐              ┌───────┐    ┌─────────┐              ┌───────────┐    ┌─────────┐
│✗ DROP │    │✓ STILL  │              │✗ BAD  │    │✓ VALID  │              │✗ WRONG    │    │✓ CORRECT│
│       │    │ HAS IT  │              │ SIG   │    │         │              │ RECIPIENT │    │         │
│Error: │    │         │              │       │    │         │              │           │    │         │
│"A can │    │Continue │              │[END]  │    │Continue │              │"Payment   │    │Continue │
│no     │    │         │              └───────┘    └─────────┘              │must go to │    │         │
│longer │    │         │                                                    │challenger"│    │         │
│cover" │    │         │                                                    │           │    │         │
│       │    │         │                                                    │[END]      │    │         │
...
└───────┘    └─────────┘
                                            ...
                                            │                               │
                                            │ - Store both signed payloads  │
                                            │ - Create Match via MatchService│
                                            │ - Match status → 'active'     │
                                            │ - Notify both players         │
                                            │                               │
                                            │ [GAME STARTS →]               │
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                      GAME IN PROGRESS (Match status: 'active')                  │
│                      Both signatures stored securely on server                  │
└─────────────────────────────────────────────────────────────────────────────────┘
                                    │
        ┌───────────────────────────┼───────────────────────────────┐
        │                           │                               │
        ▼                           ▼                               ▼
┌───────────────┐           ┌───────────────┐               ┌───────────────────┐
│ NORMAL PLAY   │           │ FORFEIT       │               │ DISCONNECT        │
│               │           │               │               │                   │
│ Players make  │           │ Player clicks │               │ Player loses      │
│ legal moves   │           │ forfeit btn   │               │ connection        │
└───────────────┘           └───────────────┘               └───────────────────┘
        │                           │                               │
        │                           ▼                               ▼
        │                   ┌───────────────────┐           ┌───────────────────┐
        │                   │ SERVER: Forfeit   │           │ 60 second grace   │
        │                   │                   │           │ period starts     │
        │                   │ A forfeits →      │           └───────────────────┘
        │                   │ Winner = B        │                   │
        │                   │                   │          ┌────────┴────────┐
        ...
        ▼
┌───────────────────────────────────────────────────────────────────────────────────┐
│                           GAME OUTCOMES (Server Determines)                       │
└───────────────────────────────────────────────────────────────────────────────────┘
        │
        ├───────────────────────────┬───────────────────────────┐
        ▼                           ▼                           ▼
...
│                   │       │                   │       │                   │
│ Server determines │       │ Server determines │       │ Server determines │
│ A is winner       │       │ B is winner       │       │ no winner         │
│                   │       │                   │       │                   │
│ Settlement:       │       │ Settlement:       │       │ No settlement     │
│ Execute B's       │       │ Execute A's       │       │ Both signatures   │
│ signed payment    │       │ signed payment    │       │ discarded         │
│ B → A             │       │ A → B             │       │                   │
│                   │       │                   │       │ Notify: "Draw,    │
│ [SETTLE →]        │       │ [SETTLE →]        │       │ no tokens moved"  │
└───────────────────┘       └───────────────────┘       │                   │
                                                        │ [FLOW ENDS]       │
                                                        └───────────────────┘
...
└───────────────────────────────────────────────────────────────────────────────────┘
        ...
        ▼                           ▼                           ▼
┌───────────────────┐       ┌───────────────────┐       ┌───────────────────┐
│ A DISCONNECTS     │       │ B DISCONNECTS     │       │ BOTH DISCONNECT   │
│                   │       │                   │       │                   │
│ 60s grace period  │       │ 60s grace period  │       │ 60s grace period  │
└───────────────────┘       └───────────────────┘       └───────────────────┘
        │                           │                           │
   ┌────┴────┐                 ┌────┴────┐                 ┌────┴────┐
   ▼         ▼                 ▼         ▼                 ▼         ▼
┌──────┐  ┌──────┐         ┌──────┐  ┌──────┐         ┌──────┐  ┌────────┐
│A back│  │Timeout│         │B back│  │Timeout│         │1 back│  │Neither │
│      │  │      │         │      │  │      │         │      │  │returns │
│Game  │  │B wins│         │Game  │  │A wins│         │Game  │  │        │
│goes  │  │      │         │goes  │  │      │         │goes  │  │DRAW    │
│on    │  │[SET] │         │on    │  │[SET] │         │on    │  │No settle│
└──────┘  └──────┘         └──────┘  └──────┘         └──────┘  └────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                      SETTLEMENT (Server-Initiated Only)                         │
│                      WagerSettlementService.settleWager(match)                  │
└─────────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
                    ┌───────────────────────────────┐
              ...
              ▼                                        ▼
    ┌─────────────────────┐                ┌─────────────────────┐
    │ ✓ ALREADY SETTLED   │                │ NOT YET SETTLED     │
    │                     │                │                     │
    │ Return existing     │                │ Continue with       │
    ...
    └─────────────────────┘                           ▼
                                        ...
                                        │ FINAL BALANCE CHECK     │
                                        │                         │
                                        │ Does loser STILL have   │
                                        │ enough tokens on-chain? │
                                        └─────────────────────────┘
                                               │              │
                                  ┌────────────┘              └────────────┐
                                  ▼                                        ▼
                    ┌─────────────────────────┐              ┌─────────────────────────┐
                    │ ✗ BALANCE DROPPED       │              │ ✓ BALANCE CONFIRMED     │
                    │                         │              │                         │
                    │ Loser spent tokens      │              │ Execute SPL transfer    │
                    │ during the game!        │              │ using signed payload    │
                    │                         │              └─────────────────────────┘
                    │ Status: settlement_fail │                           │
                    │                         │                           ▼
                    │ Log for admin review:   │              ┌─────────────────────────┐
                    │ - matchId               │              │ Submit transaction      │
                    │ - loserWallet           │              │ to Solana network       │
                    │ - expectedAmount        │              └─────────────────────────┘
                    │ - actualBalance         │                      │              │
                    │                         │             ┌────────┘              └────────┐
                    │ Notify both players:    │             ▼                                ▼
                    │ "Settlement failed -    │  ┌─────────────────────────┐  ┌─────────────────────────┐
                    │ loser balance dropped"  │  │ ✗ TRANSACTION FAILED    │  │ ✓ TRANSACTION SUCCESS   │
                    │                         │  │                         │  │                         │
                    │ [FLOW ENDS - FAILED]    │  │ Network/RPC error       │  │ Store tx signature      │
                    └─────────────────────────┘  │                         │  │ Match status →          │
                                                 │ Retry logic:            │  │ 'completed'             │
                                                 │ - Wait 5 seconds        │  │                         │
                                                 │ - Retry up to 3 times   │  │ Notify winner:          │
                                                 │ - Exponential backoff   │  │ "You won X $TOKEN!"     │
                                                 │                         │  │ [Solscan link]          │
                                                 │ If all retries fail:    │  │                         │
                                                 │ - Log error             │  │ Notify loser:           │
                                                 │ - Alert admin           │  │ "You lost X $TOKEN"     │
                                                 │ - Status: pending_manual│  │ [Solscan link]          │
                                                 │                         │  │                         │
                                                 │ [FLOW ENDS - PENDING]   │  │ [FLOW ENDS - SUCCESS]   │
                                                 └─────────────────────────┘  └─────────────────────────┘
```
...
│  ┌──────────────────────────┬─────────────────────────────┬──────────────────────────────────────────────┐  │
│  │ Error                    │ When It Happens             │ How We Handle                                │  │
│  ├──────────────────────────┼─────────────────────────────┼──────────────────────────────────────────────┤  │
│  │ Invalid token address    │ User pastes bad CA          │ Block submit, show "Invalid address"        │  │
│  │ Insufficient balance     │ User doesn't have tokens    │ Show balance + Jupiter buy link             │  │
│  │ Wallet rejected sign     │ User clicks reject/cancel   │ Cancel flow, no error logged                │  │
│  │ Signature invalid        │ Tampering or client bug     │ Reject, log for security review             │  │
│  │ Payload mismatch         │ Amount/recipient tampered   │ Reject, log for security review             │  │
│  │ Balance dropped          │ User spent between actions  │ "Balance changed, please retry"             │  │
│  │ Nonce already used       │ Replay attack attempt       │ Reject, log security alert                  │  │
│  │ Target player offline    │ Opponent not connected      │ Challenge saved, delivered when online      │  │
│  │ Duplicate challenge      │ Already challenged this user│ "Challenge already pending"                 │  │
│  └──────────────────────────┴─────────────────────────────┴──────────────────────────────────────────────┘  │
│                                                                                                             │
...
│  │ Error                    │ When It Happens             │ How We Handle                                │  │
│  ├──────────────────────────┼─────────────────────────────┼──────────────────────────────────────────────┤  │
│  │ Challenge expired        │ 5 min timeout reached       │ Auto-cleanup job, notify challenger          │  │
│  │ Challenge declined       │ B clicks decline button     │ Notify A "Challenge declined"               │  │
│  │ B has insufficient bal   │ B doesn't own the token     │ Show CA + Jupiter link + "Need X more"      │  │
│  │ B wallet rejected        │ B cancels wallet popup      │ Cancel acceptance, no error                 │  │
│  │ B signature invalid      │ Tampering or client bug     │ Reject acceptance, log security             │  │
│  │ A balance dropped        │ A spent tokens while waiting│ Cancel challenge "A can no longer cover"    │  │
│  │ Wrong recipient          │ B signed to wrong wallet    │ "Payment must be to challenger"             │  │
│  │ Challenge already gone   │ Race condition              │ "Challenge no longer exists"                │  │
│  └──────────────────────────┴─────────────────────────────┴──────────────────────────────────────────────┘  │
│                                                                                                             │
│  GAME PHASE FAILURES                                                                                        │
│  ┌──────────────────────────┬─────────────────────────────┬──────────────────────────────────────────────┐  │
│  │ Error                    │ When It Happens             │ How We Handle                                │  │
│  ├──────────────────────────┼─────────────────────────────┼──────────────────────────────────────────────┤  │
│  │ Player forfeits          │ Manual forfeit button       │ Opponent wins, settle their payment         │  │
│  │ One player disconnects   │ Network/tab close           │ 60s grace period, then opponent wins        │  │
│  │ Both players disconnect  │ Both lose connection        │ 60s grace, if neither returns → DRAW        │  │
│  │ Invalid game move        │ Client sends illegal move   │ Server rejects move, request valid one      │  │
│  │ Move timeout             │ No move for 2 minutes       │ Warn player, then forfeit if no response    │  │
│  │ Match timeout            │ No activity for 10 minutes  │ Forfeit inactive player                     │  │
│  │ Server restart mid-game  │ Server maintenance          │ Match state persisted, resume on reconnect  │  │
│  └──────────────────────────┴─────────────────────────────┴──────────────────────────────────────────────┘  │
│                                                                                                             │
...
│  │ Error                    │ When It Happens             │ How We Handle                                │  │
│  ├──────────────────────────┼─────────────────────────────┼──────────────────────────────────────────────┤  │
│  │ Loser balance dropped    │ Loser spent during game     │ Log for admin, mark settlement_failed       │  │
│  │ Transaction failed       │ Solana network issue        │ Retry 3x with backoff, then manual review   │  │
│  │ RPC timeout              │ Network congestion          │ Retry with different RPC endpoint           │  │
│  │ Double settle attempt    │ Race condition/retry        │ Idempotency check returns existing tx       │  │
│  │ Invalid stored signature │ Data corruption (rare)      │ Log error, manual admin review              │  │
│  │ Signature expired        │ Payload had expiry (if any) │ Settlement fails, admin review              │  │
│  └──────────────────────────┴─────────────────────────────┴──────────────────────────────────────────────┘  │
│                                                                                                             │
└─────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
│           │                        └──────────┘                                                             │
│           │                                                                                                 │
│           └────── cancel ────────→ ┌───────────┐ ───→ Challenger cancelled                                  │
│                   (by challenger)  │ CANCELLED │                                                            │
│                                    └───────────┘                                                            │
│                                                                                                             │
│  MATCH LIFECYCLE                                                                                            │
│  ───────────────                                                                                            │
...
│          │                                                             └───────────────────┘                │
│          │                                                                                                  │
│          └────── draw ────────────→ ┌───────────┐ ───→ No settlement needed                                 │
│                  (both disconnect)  │ COMPLETED │                                                           │
│                                     └───────────┘                                                           │
│                                                                                                             │
...
│                                                                                                             │
│      ┌─────────┐                                                                                            │
│      │ PENDING │ ←── Match ended, settlement queued                                                         │
│      └────┬────┘                                                                                            │
│           │                                                                                                 │
│           └────── process ───────→ ┌────────────┐                                                           │
│                                    │ PROCESSING │ ───┬──→ [tx success] ──→ ┌───────────┐                    │
│                                    └────────────┘    │                     │ COMPLETED │ + txHash           │
│                                                      │                     └───────────┘                    │
│                                                      │                                                      │
│                                                      ├──→ [balance drop] ─→ ┌────────┐                      │
...
challengerSignedPayload: String,  // x402 pre-signed payment: Challenger → Opponent
opponentSignedPayload: String,    // x402 pre-signed payment: Opponent → Challenger (filled on accept)
```
...
player1SignedPayload: String,  // x402 signed payment: Player 1 → Player 2
player2SignedPayload: String,  // x402 signed payment: Player 2 → Player 1
...
## 🔒 Security Model: Server Authority
...
|----------------|-------------------|
| ❌ Claim false balance | Server queries chain directly via `SolanaPaymentService.getTokenBalance()` |
| ❌ Forge x402 signatures | Server verifies with `nacl.sign.detached.verify()` |
| ❌ Declare themselves winner | Only `MatchService.endMatch()` determines winner |
| ❌ Trigger own payout | Only server calls `WagerSettlementService.settleWager()` |
| ❌ Modify wager after signing | x402 payload is immutable, signature invalidates on change |
| ❌ Replay old x402 signatures | Unique nonce per payload, server tracks used nonces |
| ❌ Settle before game ends | x402 settlement only called from `match_end` handler |
| ❌ Double-settle | Match status check + idempotency key on x402 settlement |
...
class WagerSettlementService {
    async settleWager(match) {
        ...
        if (BigInt(loserBalance) < BigInt(match.wagerToken.amountRaw)) {
            ...
            console.error(`⚠️ x402 settlement failed: ${loserWallet} balance dropped`);
            return { 
                success: false, 
...
│                        CLIENT (UNTRUSTED)                       │
│   - Can view game state                                         │
│   - Can sign x402 payment intents                               │
│   - Can send move requests                                      │
│   - CANNOT determine outcomes                                   │
│   - CANNOT trigger x402 settlements                             │
│   - CANNOT modify stored x402 signatures                        │
└─────────────────────────────────────────────────────────────────┘
                              │
                    WebSocket (encrypted)
                              │
...
┌─────────────────────────────────────────────────────────────────┐
│                        SERVER (TRUSTED)                         │
│   - Validates ALL client input                                  │
│   - Queries blockchain for balances                             │
│   - Verifies x402 signatures cryptographically                  │
│   - Runs game logic                                             │
│   - Determines winners                                          │
│   - Executes x402 settlements                                   │
│   - Stores audit trail                                          │
└─────────────────────────────────────────────────────────────────┘
                              │
                       x402 RPC calls
