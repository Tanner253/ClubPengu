...
export default [
  ...
  },
  {
    ...
    description:
      "Quality-of-life update — clearer multichain tokenomics and onboarding, guest access to inbox and settings, fixed PvP win screens for guests, and a steadier daily bonus timer. Same game on Robinhood ($WADDLE) and Solana ($CP); no major new systems this patch.",
    stats: { filesChanged: 32, additions: 990, deletions: 880 },
    changes: [
      // ── Tokenomics & first-run experience ──
      { type: "improvement", text: "In-game Tokenomics guide — $WADDLE on Robinhood Chain up front; $CP on Solana as the parallel legacy economy (Blockscout, Jupiter, and whitepaper links)" },
      { type: "improvement", text: "Penguin Maker / waddle.bet entry — $WADDLE contract + Robinhood banner marked LIVE; Solana $CP kept as the parallel chain; purple TOKENOMICS button (same economy rules on both chains)" },
      { type: "improvement", text: "Tutorial rewritten for multichain — explains $WADDLE and $CP ($WADDLE first); new guests default to the Robinhood visual rail; tutorial auto-shows on first visit" },
      { type: "improvement", text: "Creator pitch & entry token modal — multichain copy in all 10 languages; emerald styling on Robinhood flagship sections" },

      // ── Guest quality of life ──
      { type: "feature", text: "Guests can open Inbox and Settings from the HUD — browse challenge requests and read guides without connecting a wallet" },
      { type: "fix", text: "Guest gold display — inbox, profile, and wager screens show 0g (removed misleading 1,000g dev placeholder)" },
      { type: "fix", text: "PvP challenge results — guests who win now see VICTORY instead of DEFEAT (Tic-Tac-Toe, Connect 4, Card Jitsu, Blackjack, UNO, Monopoly, Battleship)" },

      // ── Daily bonus & settings ──
      { type: "fix", text: "Daily bonus playtime — opening the bonus modal no longer resets progress toward the 60-minute play requirement" },
      { type: "improvement", text: "Settings — removed DEX Screener shortcut; use Tokenomics for the economy guide and official explorer links" },

      // ── Under the hood ──
      { type: "fix", text: "Match end screens — win/loss now follows server winnerPlayerId; board symbols (X/O, R/Y) stay correct on the final board" },
      { type: "fix", text: "P2P minigame module loading — resolved challenge import issue that could block Monopoly and other wager games in dev" },
      { type: "backend", text: "571 automated tests passing — includes guest win/loss settlement coverage (204 client + 367 server)" },
    ],
  },
  {
    version: "1.4.1",
