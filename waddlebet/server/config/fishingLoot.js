 ...
 */
export function rollCatchAtDepth(depthM, rodItemId = 'basic_rod', catchConfigOverride = null) {
    ...
    const pool = getFishPoolAtDepth(effectiveDepth);
    ...
    const weights = pool.map((fish) => 1 + Math.max(0, (fish.tier || 1) - 1) * tierBias);
