/**
 * SpaceOccupancySystem - Handles MapleStory-style space occupancy banner sprites
 * Extracted from VoxelWorld.jsx for maintainability
 * 
 * Now supports dynamic space data with:
 * - Custom banner titles, tickers, descriptions
 * - Lock status display (private/public/token-gated/entry-fee)
 * - Owner information
 */

import { SPACE_BANNER_STYLES, SPACE_BANNER_CONTENT } from '../config';

/**
 * Get effective style from space data (custom colors) or preset style
 * @param {Object} style - Preset style from SPACE_BANNER_STYLES (based on styleIndex)
 * @param {Object} spaceData - Dynamic space data with banner customization
 * @returns {Object} - Effective style to use for rendering
 */
function getEffectiveStyle(style, spaceData) {
    const banner = spaceData?.banner;
    ...
    // The style parameter already comes from SPACE_BANNER_STYLES[styleIndex]
    return style;
}

/**
 * Get effective font from space data or default
 * Font is ALWAYS applied regardless of useCustomColors toggle
 * @param {Object} spaceData - Dynamic space data with banner customization
 * @returns {string} - Font family string
 */
function getEffectiveFont(spaceData) {
    const banner = spaceData?.banner;
 ...
 * @param {number} spaceIndex - Index for special styling
 * @param {number} animPhase - Animation phase (0-1) for animated banners
 * @param {Object} spaceData - Optional dynamic space data from server
 */
function drawBannerBackground(ctx, style, w, h, spaceIndex = 0, animPhase = 0, spaceData = null) {
    const padding = 12;
    const cornerRadius = 16;
    const isNightclub = spaceIndex === 2; // SKNY GANG nightclub
    
    // Get effective style (custom colors or preset)
    const effectiveStyle = getEffectiveStyle(style, spaceData);
 ...
 */
function calculateBannerHeight(ctx, spaceData, fontFamily, baseWidth = 280) {
    const padding = 16;
    ...
    const title = spaceData?.banner?.title || '';
    const ticker = spaceData?.banner?.ticker || '';
    const shill = spaceData?.banner?.shill || '';
    const owner = spaceData?.ownerUsername || '';
 ...
 */
function drawBannerContent(ctx, style, content, w, h, spaceData = null) {
    const padding = 16;
    ...
    const effectiveStyle = getEffectiveStyle(style, spaceData);
    const fontFamily = getEffectiveFont(spaceData);
    const textAlign = getEffectiveTextAlign(spaceData);
    
    // Gather content
    const title = spaceData?.banner?.title || content.title || '';
    const ticker = spaceData?.banner?.ticker || content.ticker || '';
    const shill = spaceData?.banner?.shill || content.shill || '';
    const owner = spaceData?.ownerUsername || content.owner || '';
 ...
 * @param {number} w - Canvas width
 * @param {number} h - Canvas height
 * @param {Object} spaceData - Optional dynamic space data for access status
 */
function drawPenguinCount(ctx, style, count, w, h, spaceData = null) {
    const padding = 12;
    ...
    // Access status indicator based on space data
    ctx.font = '12px Arial, sans-serif';
    ctx.textAlign = 'right';
    
    if (spaceData) {
        const accessType = spaceData.accessType || 'private';
        const hasEntryFee = spaceData.hasEntryFee;
        const hasTokenGate = spaceData.hasTokenGate;
        const isRented = spaceData.isRented;
        
        // Not rented and not reserved - show "FOR RENT"
        if (!isRented && !spaceData.isReserved) {
        ...
        // Token gated
            ...
            const tokenInfo = spaceData.tokenGateInfo;
        ...
        // Entry fee required
            ...
            const feeText = spaceData.entryFeeAmount ? `💰 ${spaceData.entryFeeAmount}` : '💰 FEE';
        ...
        // No space data - use default status
        ctx.fillStyle = count > 0 ? '#22c55e' : '#888888';
        ctx.fillText(count > 0 ? '● OPEN' : '○ EMPTY', w - padding - 15, h - 22);
    }
}
...
/**
 * Render the full space banner to a canvas
 * @param {CanvasRenderingContext2D} ctx 
 * @param {number} count - Occupancy count
 * @param {number} spaceIndex - Index for style/content selection
 * @param {Object} spaceData - Optional dynamic space data from server
 */
export function renderSpaceBanner(ctx, count, spaceIndex = 0, spaceData = null) {
    const canvas = ctx.canvas;
    const w = canvas.width;
    const h = canvas.height;
    
    // Use style from space data if available, otherwise use index
    const styleIndex = spaceData?.banner?.styleIndex ?? spaceIndex;
    const style = SPACE_BANNER_STYLES[styleIndex % SPACE_BANNER_STYLES.length];
    const content = SPACE_BANNER_CONTENT[spaceIndex % SPACE_BANNER_CONTENT.length];
    
    // Clear canvas
    ctx.clearRect(0, 0, w, h);
    
    // Draw all components with space data
    drawBannerBackground(ctx, style, w, h, spaceIndex, 0, spaceData);
    drawBannerContent(ctx, style, content, w, h, spaceData);
    drawPenguinCount(ctx, style, count, w, h, spaceData);
}

/**
 * Create a new space occupancy sprite
 ...
 */
export function createSpaceOccupancySprite(THREE, count, spaceIndex = 0, spaceData = null) {
    const w = 280;
    ...
    const fontFamily = getEffectiveFont(spaceData);
    const h = calculateBannerHeight(tempCtx, spaceData, fontFamily, w);
    ...
    renderSpaceBanner(ctx, count, spaceIndex, spaceData);
    ...
    sprite.position.y = 10; // Higher above space
    sprite.renderOrder = 998;
    sprite.visible = false; // Start hidden, show when player is close
    
    // Store index and data for updates
    sprite.userData.styleIndex = spaceIndex;
    sprite.userData.spaceIndex = spaceIndex;
    sprite.userData.spaceData = spaceData;
    sprite.userData.canvasHeight = h;
    
    // Store banner data for zoom overlay
    sprite.userData.bannerData = {
        type: 'canvas',
        title: spaceData?.banner?.title || 'Space Instructions',
        description: 'Click to view space rental instructions',
        canvas: canvas,
        renderFn: (ctx, w, h) => {
            renderSpaceBanner(ctx, count, spaceIndex, spaceData);
        }
...
/**
 * Update an existing space occupancy sprite
 * @param {Object} THREE - THREE.js library
 * @param {THREE.Sprite} sprite - The sprite to update
 * @param {number} count - New occupancy count
 * @param {Object} spaceData - Optional updated space data from server
 */
export function updateSpaceOccupancySprite(THREE, sprite, count, spaceData = null) {
    if (!sprite || !sprite.material) return;
    
    const spaceIndex = sprite.userData.spaceIndex || 0;
    // Use new data if provided, otherwise use stored data
    const data = spaceData || sprite.userData.spaceData || null;
    ...
    const ctx = canvas.getContext('2d');
    renderSpaceBanner(ctx, count, spaceIndex, data);
...
export default {
    createSpaceOccupancySprite,
    updateSpaceOccupancySprite,
    renderSpaceBanner
};
