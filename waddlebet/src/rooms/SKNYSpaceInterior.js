/**
 * SKNY GANG Space Interior - Miniature Nightclub
 * Features: Disco ball, LED strips, DJ booth, VIP seating, dance floor, neon signs
 */

/**
 * Generate the SKNY GANG nightclub space interior
 ...
 */
export function generateSKNYSpaceInterior(THREE, scene) {
    const SPACE_SIZE = 30;
    ...
    const floor = new THREE.Mesh(
        new THREE.CircleGeometry(SPACE_SIZE / 2, 64), 
        floorMat
    );
    ...
    const dome = new THREE.Mesh(
        new THREE.SphereGeometry(SPACE_SIZE / 2, 48, 32, 0, Math.PI * 2, 0, Math.PI / 2),
        domeMat
    );
    dome.scale.y = 0.65;
    scene.add(dome);
    ...
    for (let i = 0; i < 4; i++) {
        const ringY = 2 + i * 2;
        const ringRadius = (SPACE_SIZE / 2 - 1) * Math.cos(Math.asin(ringY / (SPACE_SIZE / 2 * 0.65)));
    ...
    signGroup.position.set(0, 4, -SPACE_SIZE / 2 + 5);
    scene.add(signGroup);
    ...
    doorGroup.position.set(0, 0, SPACE_SIZE / 2 - 0.5);
    scene.add(doorGroup);
...
export default generateSKNYSpaceInterior;
