/**
 * SpaceInfoBoard - Information board explaining space mechanics
 * Shows rental costs, owner benefits, and visitor info
 * HORIZONTAL LAYOUT with frame and spotlights
 */

export function createSpaceInfoBoard(THREE, position = { x: 0, y: 0, z: 0 }, rotation = 0) {
    ...
    drawSpaceInfoBoard(ctx, canvas.width, canvas.height);
    ...
    surface.userData.bannerData = {
        type: 'canvas',
        title: '🏠 Space Rental Guide',
        description: 'Information about space rental mechanics, owner perks, and visitor access',
        canvas: canvas,
        renderFn: (ctx, w, h, isMobilePortrait = false) => {
            if (isMobilePortrait) {
                drawSpaceInfoBoardMobile(ctx, w, h);
            } else {
                drawSpaceInfoBoard(ctx, w, h);
            }
        }
    };
...
function drawSpaceInfoBoard(ctx, W, H) {
    ...
    ctx.shadowBlur = 50;
    ctx.fillText('🏠 SPACE RENTAL GUIDE 🏠', W/2, 175);
    ctx.shadowBlur = 0;
    ...
    const rentInfo = [
        ...
        { label: 'Total Spaces:', value: '10 available', color: '#98FB98' },
    ];
    ...
    ctx.textAlign = 'center';
    ctx.fillText('🎉 Rent an space to advertise & earn from visitors! 🎉', W/2, H - 80);
    ctx.shadowBlur = 0;
}

// Mobile portrait version - vertical layout with larger text
function drawSpaceInfoBoardMobile(ctx, W, H) {
    // Background gradient - icy blue
    ...
    ctx.shadowColor = '#87CEEB';
    ctx.shadowBlur = 20;
    ctx.fillText('🏠 SPACE RENTAL GUIDE 🏠', W/2, 60);
    ctx.shadowBlur = 0;
    ...
    ctx.font = 'bold 20px "Segoe UI", Arial';
    ctx.textAlign = 'center';
    ctx.fillText('🎉 Rent an space to advertise & earn! 🎉', W/2, H - 30);
    ctx.shadowBlur = 0;
}

export default createSpaceInfoBoard;
