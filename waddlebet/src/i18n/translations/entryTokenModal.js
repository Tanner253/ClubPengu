...
export default {
    ...
    'entryToken.robinhoodBody': row(
        '$WADDLE is deployed on Robinhood Chain (EVM). Paste the contract address into MetaMask or your DEX — full multichain wagers and economy ship in phases.',
        '$WADDLE 已部署于 Robinhood Chain (EVM)。将合约地址粘贴到 MetaMask 或 DEX——完整多链赌注与经济将分阶段上线。',
        '$WADDLE está desplegado en Robinhood Chain (EVM). Pega la CA en MetaMask o tu DEX — economía multichain en fases.',
        '$WADDLE implantado na Robinhood Chain (EVM). Cole o CA no MetaMask ou DEX — economia multichain em fases.',
        'Robinhood Chain(EVM)에 $WADDLE 배포됨. MetaMask/DEX에 CA 붙여넣기 — 멀티체인 경제 단계적 출시.',
        'Robinhood Chain（EVM）に$WADDLEデプロイ済み。MetaMask/DEXにCAを貼り付け——マルチチェーンは段階的に。',
        '$WADDLE déployé sur Robinhood Chain (EVM). Collez le CA dans MetaMask ou votre DEX — économie multichaîne par phases.',
        '$WADDLE auf Robinhood Chain (EVM) deployed. CA in MetaMask oder DEX einfügen — Multichain-Ökonomie in Phasen.',
        '$WADDLE развёрнут на Robinhood Chain (EVM). Вставьте CA в MetaMask или DEX — мультичейн поэтапно.',
        '$WADDLE منشور على Robinhood Chain (EVM). الصق CA في MetaMask أو DEX — اقتصاد متعدد السلاسل على مراحل.'
    ),
