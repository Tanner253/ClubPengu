...
const CHANGELOG = [
    {
        ...
        description: 'Gold scarcity tuning, fishing hole stock, NPC daily contracts, 7-day $CP streak, Today HUD, Tarkov merchant UI, PvE blackjack, puffle shop, economy guide. 137 files.',
        highlight: true,
        stats: { filesChanged: 137, additions: 10149, deletions: 1894 },
        changes: [
            { type: 'backend', text: 'goldEconomy.js — 10g start, ferry/bait/wager caps, mint recipes, v2 migration (10% retention)' },
            { type: 'fix', text: 'Emergency sell floor(unit×ratio×qty) — bulk 1g exploit closed' },
            { type: 'improvement', text: 'Onboarding reward 10g (was 500g); solo minigames no longer mint gold' },
            { type: 'feature', text: 'FishingHoleService — per-hole tier stock depletes & regrows server-side' },
            { type: 'feature', text: 'WormForageService — mossy log worm foraging in forest' },
            { type: 'feature', text: 'treeWoodSpecies — pine/birch/oak/ironwood yield mapping' },
            { type: 'fix', text: 'Loaner basic axe on first forest ferry arrival' },
            { type: 'feature', text: 'NpcDailyOrderService — Salty catch + Clive timber daily contracts' },
            { type: 'feature', text: 'DailyQuestHUD — Today panel: contracts, streak calendar, 60 min bar, claim' },
            { type: 'feature', text: 'NPC flow — accept at merchant → track HUD → turn in' },
            { type: 'feature', text: '7-day streak — $CP days 1/2/4/5/7 (1k→5k); gold-only days 3 & 6' },
            { type: 'feature', text: 'NpcDialogueModal Tarkov UI — tabs, speech, TraderOfferCanvas art' },
            { type: 'feature', text: 'MaterialBreakdown progress bars on recipes & contracts' },
            { type: 'mobile', text: 'Merchant UI — 2-col grid, scrollable detail, touch targets' },
            { type: 'backend', text: 'GameInventoryService — loaner axe, mint, order turn-in, sell fix' },
            { type: 'feature', text: 'PveBlackjackService — server-authoritative casino blackjack' },
            { type: 'feature', text: 'PuffleShopTabs + canvas preview + accessories' },
            { type: 'feature', text: 'EconomyGuideModal in Settings — gold vs pebbles vs $CP' },
            { type: 'feature', text: 'CreatorPitchModal + TOKENOMICS badge on Penguin Maker' },
            { type: 'improvement', text: 'WHITEPAPER / OPEN SOURCED animated header badges' },
            { type: 'backend', text: 'MultiplayerContext — quest accept/turn-in, streak claim, daily status' },
            { type: 'backend', text: '+8 test files — orders, fishing holes, gold economy, streak, blackjack' },
            { type: 'content', text: 'Whitepaper canvas diagrams + economics rewrite; README overhauls' },
            { type: 'fix', text: 'Today HUD gold-day preview for streak days 3 & 6' },
        ],
    },
    {
        version: '1.3.2',
