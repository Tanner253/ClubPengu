/**
 * SpaceSettingsPanel - Owner control panel for space settings
 * Manage access control, entry fees, token gates, and banner customization
 */

import React, { useState, useEffect, useCallback } from 'react';
import { SPACE_BANNER_STYLES } from '../config/roomConfig.js';
import { useSpace } from '../space/SpaceContext.jsx';
import { paySpaceRent } from '../wallet/SolanaPayment.js';
import { RENT_WALLET_ADDRESS, CPW3_TOKEN_ADDRESS, SPACE_CONFIG } from '../config/solana.js';
...
const SpaceSettingsPanel = ({ 
    isOpen, 
    onClose, 
    spaceData,
    onSave
}) => {
    const { updateSettings: sendSettings, payRent: sendPayRent, isLoading: contextLoading } = useSpace();
    
    const [settings, setSettings] = useState({
        spaceId: null, // Track which space these settings are for
        accessType: 'private',
        tokenGate: {
            enabled: false,
            tokenAddress: '',
    ...
    useEffect(() => {
        if (spaceData && !isSaving) {
            // Only load from server data if:
            // 1. This is the initial load (panel just opened), OR
            // 2. The spaceId changed (different space selected)
            const shouldLoad = !hasLoadedInitial || 
                (spaceData.spaceId && settings.spaceId !== spaceData.spaceId);
            
            if (shouldLoad) {
                console.log('🏠 [SettingsPanel] Loading spaceData:', spaceData);
                console.log('🏠 [SettingsPanel] Banner from server:', spaceData.banner);
                
            setSettings({
                    spaceId: spaceData.spaceId, // Track which space we loaded
                accessType: spaceData.accessType || 'private',
                tokenGate: {
                    enabled: spaceData.tokenGate?.enabled || false,
                    tokenAddress: spaceData.tokenGate?.tokenAddress || '',
                    tokenSymbol: spaceData.tokenGate?.tokenSymbol || '',
                    minimumBalance: spaceData.tokenGate?.minimumBalance || 1
                },
                entryFee: {
                    enabled: spaceData.entryFee?.enabled || false,
                    amount: spaceData.entryFee?.amount || 0,
                    tokenAddress: spaceData.entryFee?.tokenAddress || '',
                    tokenSymbol: spaceData.entryFee?.tokenSymbol || ''
                },
                banner: {
                    title: spaceData.banner?.title || '',
                    ticker: spaceData.banner?.ticker || '',
                    shill: spaceData.banner?.shill || '',
                        styleIndex: spaceData.banner?.styleIndex ?? 0,
                        // Use explicit checks for boolean/array fields
                        useCustomColors: spaceData.banner?.useCustomColors === true,
                        customGradient: Array.isArray(spaceData.banner?.customGradient) && spaceData.banner.customGradient.length === 3
                            ? spaceData.banner.customGradient 
                            : ['#845EF7', '#BE4BDB', '#F06595'],
                        textColor: spaceData.banner?.textColor || '#FFFFFF',
                        accentColor: spaceData.banner?.accentColor || '#00FFFF',
                        font: spaceData.banner?.font || 'Inter, system-ui, sans-serif',
                        textAlign: spaceData.banner?.textAlign || 'center'
                }
            });
                setHasLoadedInitial(true);
            }
        }
    }, [spaceData, isSaving, hasLoadedInitial]);
    ...
    const handleSave = useCallback(() => {
        ...
        // Build settings without the spaceId tracker
        const { spaceId: _, ...settingsToSend } = settings;
        
        // Use WebSocket via SpaceContext
        console.log('🏠 Saving space settings:', settingsToSend);
        console.log('🏠 Banner being saved:', settingsToSend.banner);
        sendSettings(spaceData.spaceId, settingsToSend);
    ...
    }, [spaceData, settings, sendSettings]);
    
    /**
     * Handle rent payment for past due spaces
     */
    const handlePayRent = useCallback(async () => {
        ...
        try {
            ...
            console.log(`   Space: ${spaceData.spaceId}`);
            console.log(`   Amount: ${SPACE_CONFIG.DAILY_RENT_CPW3} $WADDLE`);
            
            // Step 1: Send the Solana transaction
            const paymentResult = await paySpaceRent(
                spaceData.spaceId,
                SPACE_CONFIG.DAILY_RENT_CPW3,
                RENT_WALLET_ADDRESS,
                CPW3_TOKEN_ADDRESS
            );
            ...
            sendPayRent(spaceData.spaceId, paymentResult.signature);
    ...
    }, [spaceData, sendPayRent]);
    
    if (!isOpen) return null;
    
    // Check if we have full space data (not just the ID)
    const isDataLoading = !spaceData?.accessType && !spaceData?.ownerWallet;
            ...
            <div className="relative z-10 w-full max-w-lg mx-4 bg-gradient-to-b from-slate-800 to-slate-900 rounded-2xl shadow-2xl border border-purple-500/30 overflow-hidden max-h-[90vh] flex flex-col">
                ...
                <div className="relative px-6 py-4 bg-gradient-to-r from-purple-600/20 to-pink-600/20 border-b border-white/10 shrink-0">
                    ...
                    <p className="text-sm text-slate-400 mt-1">
                        {spaceData?.spaceId?.replace('space', 'Space ')}
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="Short tagline for your space..."
                                    maxLength={60}
                                    rows={2}
                            ...
                            {!settings.banner.useCustomColors && (
                                ...
                                <div className="grid grid-cols-4 gap-2">
                                    {SPACE_BANNER_STYLES.slice(0, 8).map((style, idx) => (
                                        <button
                                            key={idx}
                                            onClick={() => setSettings({
                                                ...settings,
                                ...
                                <div 
                                    ...
                                    style={{
                                        background: settings.banner.useCustomColors
                                            ...
                                            : `linear-gradient(180deg, ${SPACE_BANNER_STYLES[settings.banner.styleIndex ?? 0]?.bgGradient[0] || '#333'}, ${SPACE_BANNER_STYLES[settings.banner.styleIndex ?? 0]?.bgGradient[2] || '#111'})`,
                                        textAlign: settings.banner.textAlign || 'center',
                                        border: `3px solid ${settings.banner.useCustomColors ? settings.banner.accentColor : SPACE_BANNER_STYLES[settings.banner.styleIndex ?? 0]?.accentColor || '#0ff'}`,
                                        boxShadow: `0 0 15px ${settings.banner.useCustomColors ? settings.banner.accentColor : SPACE_BANNER_STYLES[settings.banner.styleIndex ?? 0]?.accentColor || '#0ff'}40`
                                    }}
                                >
                                    <div 
                                        ...
                                        style={{ 
                                            color: settings.banner.useCustomColors 
                                                ...
                                                : SPACE_BANNER_STYLES[settings.banner.styleIndex ?? 0]?.textColor || '#fff',
                                            fontFamily: settings.banner.font || 'Inter, system-ui, sans-serif'
                                        }}
                                    >
                                        {settings.banner.title || 'Your Space'}
                                    </div>
                                        ...
                                        <div 
                                            ...
                                            style={{ 
                                                color: settings.banner.useCustomColors 
                                                    ...
                                                    : SPACE_BANNER_STYLES[settings.banner.styleIndex ?? 0]?.accentColor || '#0ff',
                                                fontFamily: settings.banner.font || 'Inter, system-ui, sans-serif'
                                            }}
                                        >
                                            {settings.banner.ticker}
                            ...
                            <div className="bg-slate-700/50 rounded-lg p-4 space-y-3">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className={`font-semibold ${
                                        spaceData?.rentStatus === 'current' ? 'text-green-400' :
                                        spaceData?.rentStatus === 'grace_period' ? 'text-amber-400' :
                                        'text-red-400'
                                    }`}>
                                        {spaceData?.rentStatus === 'current' ? '✅ Rent Paid' :
                                         spaceData?.rentStatus === 'grace_period' ? '⚠️ Payment Due' :
                                         '❌ Overdue'}
                                    </span>
                                </div>
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className={`${spaceData?.isReserved ? 'text-purple-400' : 'text-white'}`}>
                                        {spaceData?.isReserved 
                                            ? '✨ Pre-Paid' 
                                            : spaceData?.rentDueDate 
                                            ? new Date(spaceData.rentDueDate).toLocaleString()
                                            : 'N/A'}
                                    </span>
                                </div>
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-yellow-400 font-mono">
                                        {spaceData?.stats?.totalRentPaid?.toLocaleString() || 0} $WADDLE
                                    </span>
                                </div>
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-white">
                                        {spaceData?.stats?.uniqueVisitors || 0}
                                    </span>
                                </div>
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-green-400 font-mono">
                                        {spaceData?.stats?.totalEntryFeesCollected?.toLocaleString() || 0} {spaceData?.entryFee?.tokenSymbol || 'tokens'}
                                    </span>
                                </div>
                            </div>
                            
                            {/* Rent Payment Required Warning */}
                            {(spaceData?.rentStatus === 'grace_period' || spaceData?.rentStatus === 'overdue') && (
                                <div className={`border rounded-lg p-4 ${
                                    spaceData?.rentStatus === 'overdue' 
                                        ? 'bg-red-500/20 border-red-500/40' 
                                        : 'bg-amber-500/20 border-amber-500/40'
                                }`}>
                                    <p className={`font-semibold ${
                                        spaceData?.rentStatus === 'overdue' ? 'text-red-400' : 'text-amber-400'
                                    }`}>
                                        {spaceData?.rentStatus === 'overdue' 
                                    ...
                                    <p className="text-sm text-slate-300 mt-1">
                                        {spaceData?.rentStatus === 'overdue'
                                            ? 'Pay immediately to keep your space!'
                                    ...
                                    >
                                        {isPayingRent 
                                            ...
                                            : rentPaymentSuccess 
                                                ? '✅ Payment Sent!'
                                                : `💰 Pay Rent (${SPACE_CONFIG.DAILY_RENT_CPW3.toLocaleString()} $WADDLE)`}
                                    </button>
                                </div>
                            )}
                            
                            {/* Rent is current - show status */}
                            {spaceData?.rentStatus === 'current' && (
                                <div className="bg-green-500/20 border border-green-500/40 rounded-lg p-4">
                                    <p className="text-green-400 font-semibold">✅ Rent Status: Current</p>
                                    <p className="text-sm text-slate-300 mt-1">
                                        Your rent is paid. Next payment due: {spaceData?.rentDueDate ? new Date(spaceData.rentDueDate).toLocaleString() : 'Unknown'}
                                    </p>
                                </div>
                            )}
                        </div>
