/**
 * SpaceRequirementsPanel - Shows entry requirements for a rented space
...
import { paySpaceEntryFee } from '../wallet/SolanaPayment.js';
...
const SpaceRequirementsPanel = ({ 
    ...
    // Extract space data safely (before any hooks that depend on them)
    const spaceId = spaceData?.spaceId;
    const ownerUsername = spaceData?.ownerUsername;
    const ownerWallet = spaceData?.ownerWallet;
    const accessType = spaceData?.accessType;
    const hasTokenGate = spaceData?.hasTokenGate;
    const hasEntryFee = spaceData?.hasEntryFee;
    const tokenGate = spaceData?.tokenGate;
    const tokenGateInfo = spaceData?.tokenGateInfo;
    const entryFee = spaceData?.entryFee;
    const entryFeeToken = spaceData?.entryFeeToken;
    const entryFeeAmount = spaceData?.entryFeeAmount;
    ...
    const handleRefreshStatus = useCallback(() => {
        if (!spaceId || !send) return;
        setCheckingStatus(true);
        setError(null);
        send({
            type: 'space_check_requirements',
            spaceId
        });
    }, [send, spaceId]);
    ...
    const handlePayAndEnter = useCallback(async () => {
        ...
        if (needsPayment && send) {
            ...
            try {
                ...
                const eligibilityCheck = await new Promise((resolve) => {
                    ...
                    const handleResponse = (event) => {
                        try {
                            ...
                            if (msg.type === 'space_can_enter' && msg.spaceId === spaceId) {
                    ...
                    send({ type: 'space_can_enter', spaceId });
                });
                ...
                // DEAD SIMPLE: Send tokens to space owner, decimals fetched automatically
                const paymentResult = await paySpaceEntryFee(
                    spaceId,
                    feeAmount,
                    ownerWallet,
                    feeTokenAddress
    ...
    }, [walletAddress, needsPayment, allRequirementsMet, spaceId, feeAmount, ownerWallet, feeTokenAddress, send, onEnterSuccess, onClose]);
    
    // Reset state when panel opens/closes or space changes
    // ALWAYS fetch fresh status from server when panel opens (token balance may have changed)
    useEffect(() => {
        if (isOpen && spaceId && send) {
            setError(null);
            ...
            console.log('🏠 Requesting fresh requirements status for:', spaceId);
            send({
                type: 'space_check_requirements',
                spaceId
            });
    ...
    useEffect(() => {
        ...
        const handleMessage = (event) => {
            try {
                ...
                if (msg.type === 'space_requirements_status' && msg.spaceId === spaceId) {
                ...
                if (msg.type === 'space_pay_entry_result' && msg.spaceId === spaceId) {
                    setPaymentLoading(false);
                    if (msg.success) {
    ...
    }, [isOpen, spaceId, showTokenGate, hasTokenGateMet, onEnterSuccess, onClose]);
                ...
                <div className="relative px-6 py-5 bg-gradient-to-r from-purple-600/30 via-indigo-600/20 to-cyan-600/30 border-b border-white/10">
                    <div className="text-center">
                        ...
                        <p className="text-purple-300 text-sm">
                            {ownerUsername ? `${ownerUsername}'s Space` : `Space ${spaceId?.replace('space', '')}`}
                        </p>
                        {ownerWallet && (
                        ...
                        <div className="bg-slate-800/50 border border-slate-600/50 rounded-lg p-3 text-center">
                            <span className="text-xs text-slate-400">
                                {allRequirementsMet 
                                    ...
                                    : '⚠️ You must meet both requirements to enter this space'
                                }
                            </span>
                        </div>
                    )}
...
export default SpaceRequirementsPanel;
