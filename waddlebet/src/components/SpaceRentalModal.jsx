...
const SpaceRentalModal = ({ 
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (msg.type === 'space_can_rent') {
                    ...
                    setBalanceInfo({
                        ...
                        required: msg.required || SPACE_CONFIG.MINIMUM_BALANCE_CPW3
                    });
                }
            } catch (e) {
                // Ignore parse errors
        ...
        if (spaceData?.spaceId) {
            send({ type: 'space_can_rent', spaceId: spaceData.spaceId });
        }
        
        return () => {
            ws.removeEventListener('message', handleMessage);
        };
    }, [isOpen, spaceData?.spaceId, send, onRentSuccess, onClose]);
        ...
        try {
            ...
            const eligibilityCheck = await new Promise((resolve) => {
                ...
                const handleResponse = (event) => {
                    try {
                        const msg = JSON.parse(event.data);
                        if (msg.type === 'space_can_rent' && msg.spaceId === spaceData.spaceId) {
            ...
            if (!eligibilityCheck.canRent) {
                ...
                setError(eligibilityCheck.message || 'Cannot rent this space');
                setIsLoading(false);
                return;
            }
    ...
    }, [walletAddress, spaceData?.spaceId, send]);
                    ...
                    {isReserved ? (
                        <div className="bg-purple-500/20 border border-purple-500/40 rounded-lg p-4 text-center">
                            <span className="text-purple-400 font-bold">
                                🔒 Reserved Rental - {spaceData?.ownerUsername || 'Reserved Owner'}
                            </span>
                        </div>
                    ) : isRented ? (
                        <div className="bg-amber-500/20 border border-amber-500/40 rounded-lg p-4 text-center">
                            <span className="text-amber-400 font-bold">
                                🔑 Currently Rented by {spaceData.ownerUsername}
                            </span>
                        </div>
                    ) : (
                        ...
                        <>
                                ...
                                <div className="space-y-2 text-sm text-slate-300">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="text-yellow-400 font-mono">
                                            {SPACE_CONFIG.DAILY_RENT_CPW3.toLocaleString()} $WADDLE
                                        </span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span>Minimum Balance Required:</span>
                                        <span className="text-yellow-400 font-mono">
                                            {SPACE_CONFIG.MINIMUM_BALANCE_CPW3.toLocaleString()} $WADDLE
                                        </span>
                                    </div>
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="text-red-400">{SPACE_CONFIG.GRACE_PERIOD_HOURS} hours</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-700/50 rounded-lg p-4 space-y-2">
                                ...
                                <ul className="text-sm text-slate-300 space-y-1">
                                    <li className="flex items-center gap-2">
                                        ...
                                        Hold {SPACE_CONFIG.MINIMUM_BALANCE_CPW3.toLocaleString()} $WADDLE (7 days rent)
                                    </li>
                                    <li className="flex items-center gap-2">
                                        <span className="text-green-400">✓</span>
                            ...
                            >
                                {isLoading ? (
                                    ...
                                    `🔑 Rent for ${SPACE_CONFIG.DAILY_RENT_CPW3.toLocaleString()} $WADDLE/day`
                                )}
                            </button>
